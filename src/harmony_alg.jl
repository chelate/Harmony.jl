...
Z :: Matrix{Float64, d x N}
    The input embedding, to be corrected in Harmony. 
    This is often PCA embeddings of cells.
Zhat :: Matrix{Float64, d x N}  
    The integrated embedding, output by Harmony.
R :: Matrix{Float64, K x N}  
    The soft assignment matrix of cells (columns) to clusters  (rows). 
    Each column is a probability distribution and thus sums to 1.
φ :: Matrix{Float64, B x N}
    One-hot assignment matrix of cells (columns) to batches (rows).
Prb :: Vector{Float64, B }
    Frequency of batches.
O :: Matrix{Float64, K x B}
    The observed co-occurence matrix of cells in clusters (rows) and  batches (columns).
E :: Matrix{Float64, K x B}
    The expected co-occurence matrix of cells in clusters (rows) and  batches (columns).
    under independence assumption.
Y :: Matrix{Float64, d x K}
    Cluster centroid locations in the k-means clustering algorithm.
"""
...
import LogExpFunctions: xlogx, softmax!
import LinearAlgebra: normalize!, normalize
import Clustering


X = rand(50, 100000)
@time map(normalize!,eachcol(a))
result = Clustering.kmeans(X, 20; maxiter=200)
result.centers # get the centroids


function init_cluster()
end

function compute_objective(self)
end

function harmonize(Z, R, φ)
...
"""
soft clustering of corrected embedding
takes an embedding zhat, and a one hot batch indicator

returns R :: (k x n)
    a soft clustering assignment of each cell, where the columns sum to one
"""
function cluster_membership(zhat, φ;  nclust = 20, # dimension size K 
    σ = 0.1, # common var
    sigma = fill(σ, K)) # to assign a different variance to each dimension

    # get starting clusters
    result = Clustering.kmeans(zhat, nclust; 
        maxiter=200)
    Y = result.centers # get the centroids

    # L2 normalization
    map(normalize!,eachcol(Y)) # d x K
    map(normalize!,eachcol(Zhat)) # d x N
    # the distance between yk and zn
    dist_matrix = -2 * (1 .- Y'Zhat) ./ sigma  # K x N
    # normalize and exponentiate each column:
    # https://en.wikipedia.org/wiki/Softmax_function
    R = map(softmax,eachcol(dist_matrix))
...
function check_convergence(self, i_type)
end
